...
class CompoundSDK {
  ...
  async getInterestAPR(asset) {
    try {
      ...
      throw new Error(
        `Failed to get interest APR for ${asset}: ${error.message}`
      );
    }
  }
  ...
  async getCompAPR(asset) {
    try {
      ...
      this._log(
        `COMP APR for ${asset}: ${(estimatedCompAPR * 100).toFixed(2)}%`
      );
  ...
  async getTVL(chainName = null, cometAddress = null) {
    console.log('getTVL', chainName, cometAddress);
    try {
      // Use current chain if not specified
      const targetChainId = chainName
        ? this._getChainIdFromName(chainName)
        : this.chainId;
      ...
      const [totalSupplyBase, numAssets] = await Promise.all([
        comet.totalSupply(),
        comet.numAssets(),
      ]);

      console.log('totalSupplyBase', totalSupplyBase, 'numAssets', numAssets);
      ...
      for (let i = 0; i < Number(numAssets); i++) {
        try {
          ...
          if (totalSupplyAsset > 0) {
            ...
            const assetTVL =
              (Number(totalSupplyAsset.toString()) / Number(scale.toString())) *
              assetPrice;
      ...
      console.log('baseTVL', baseTVL, 'collateralTVL', collateralTVL);
      const totalTVL = baseTVL + collateralTVL;
      this._log(
        `Complete TVL (${chainDisplay}): $${totalTVL.toLocaleString()} USD`
      );
  ...
  async supply(asset, amount) {
    ...
    try {
      ...
      const tx = await comet
        .connect(wallet)
        .supply(assetInfo.underlying, amountWei);
  ...
  async withdraw(asset, amount) {
    ...
    try {
      ...
      const tx = await comet
        .connect(wallet)
        .withdraw(assetInfo.underlying, amountWei);
  ...
  async claimRewards(userAddress) {
    ...
    try {
      ...
      const tx = await rewards
        .connect(wallet)
        .claim(this.comet, userAddress, true);
  ...
  async getBalance(asset, userAddress) {
    try {
      ...
      if (compRewards > 1e20) {
        // Clearly anomalous value
        this._log(
          `Warning: Anomalous COMP reward value detected: ${compRewards.toExponential(2)}`
        );
  ...
  _getMarketConfig(chainId) {
    const config =
      chainId === 1 ? COMPOUND_MARKETS.ethereum : COMPOUND_MARKETS.base;
    if (!config) {
