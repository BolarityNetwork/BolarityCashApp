...
interface MorphoVaultContractOperations {
...
export function useMorphoVaultContract(): MorphoVaultContractOperations {
  const {
  ...
  // Use ethers.js ABI encoder for ERC-4626 methods
  const encodeFunctionCall = useCallback(
    (methodSignature: string, params: any[]): string => {
      try {
        ...
        // Fallback to simplified encoding for ERC-4626 methods
        const methodIds: { [key: string]: string } = {
          'approve(address,uint256)': '0x095ea7b3',
          'deposit(uint256,address)': '0x6e553f65',
          'withdraw(uint256,address,address)': '0xba087652',
        };
  ...
  const deposit = useCallback(
    async (params: VaultOperationParams): Promise<VaultOperationResult> => {
      ...
      try {
        ...
        if (!vault.marketAddress || vault.marketAddress === ZERO_ADDRESS) {
          throw new Error(
            'Invalid vault address: marketAddress cannot be zero address'
          );
        }
        if (!vault.asset || vault.asset === ZERO_ADDRESS) {
          throw new Error(
            'Invalid asset address: asset cannot be zero address'
          );
        }

        const receiver = await resolveTargetAddress(userAddress);
        const amountWei = parseAmount(amount, vault.decimals);

        // Approve vault asset to vault contract (marketAddress is the vault address)
        ...
        const depositData = encodeFunctionCall('deposit(uint256,address)', [
          amountWei,
          receiver,
        ]);

        // Batch approve and deposit transactions
        const txHash = await sendGaslessTransaction([
          ...
          {
            to: vault.marketAddress as `0x${string}`,
  ...
  // Morpho Vault Withdraw (ERC-4626)
  const withdraw = useCallback(
    async (params: VaultOperationParams): Promise<VaultOperationResult> => {
      ...
      try {
        ...
        const owner = await resolveTargetAddress(userAddress);
        const amountWei = parseAmount(amount, vault.decimals);

        // ERC-4626 withdraw(uint256 assets, address receiver, address owner)
        // receiver and owner are the same (user withdrawing to themselves)
        const withdrawData = encodeFunctionCall(
          'withdraw(uint256,address,address)',
          [amountWei, owner, owner]
        );
