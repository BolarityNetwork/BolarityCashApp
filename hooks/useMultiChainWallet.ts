...
const NETWORKS: Record<string, Network> = {
  mainnet: { chainId: '0x1', name: 'Ethereum Mainnet', rpcUrl: 'https://mainnet.infura.io/v3/', symbol: 'ETH', blockExplorer: 'https://etherscan.io', icon: '🔷', color: '#627eea' },
  sepolia: { chainId: '0xaa36a7', name: 'Ethereum Sepolia', rpcUrl: 'https://sepolia.infura.io/v3/', symbol: 'ETH', blockExplorer: 'https://sepolia.etherscan.io', icon: '🧪', color: '#ffa500' },
  polygon: { chainId: '0x89', name: 'Polygon Mainnet', rpcUrl: 'https://polygon-rpc.com', symbol: 'MATIC', blockExplorer: 'https://polygonscan.com', icon: '💜', color: '#8247e5' },
  bsc: { chainId: '0x38', name: 'BSC Mainnet', rpcUrl: 'https://bsc-dataseed1.binance.org', symbol: 'BNB', blockExplorer: 'https://bscscan.com', icon: '💛', color: '#f3ba2f' },
  arbitrum: { chainId: '0xa4b1', name: 'Arbitrum One', rpcUrl: 'https://arb1.arbitrum.io/rpc', symbol: 'ETH', blockExplorer: 'https://arbiscan.io', icon: '🔵', color: '#28a0f0' },
  optimism: { chainId: '0xa', name: 'Optimism', rpcUrl: 'https://mainnet.optimism.io', symbol: 'ETH', blockExplorer: 'https://optimistic.etherscan.io', icon: '🔴', color: '#ff0420' },
  base: { chainId: '0x2105', name: 'Base Mainnet', rpcUrl: 'https://rpc.ankr.com/base/d71a9cd8dd190bf86a472bb7c7211ec1d99f131c9739266c6420a2efcafe4325', symbol: 'ETH', blockExplorer: 'https://basescan.org', icon: '🟦', color: '#0052ff' }
};
...
class WalletProvider {
  ...
  async get(): Promise<any> {
    const wallets = this.activeChain === 'ethereum' ? this.ethWallets : this.solWallets;
    if (!wallets?.length) throw new Error(`No ${this.activeChain} wallet available`);
...
class Storage {
  ...
  static async save(state: Partial<WalletState>): Promise<void> {
    const promises = [];
    if (state.activeChain) promises.push(AsyncStorage.setItem(this.KEYS.CHAIN, state.activeChain));
    if (state.activeNetwork) promises.push(AsyncStorage.setItem(this.KEYS.NETWORK, state.activeNetwork));
...
class TransactionHandler {
  constructor(private provider: WalletProvider, private state: WalletState) {}
  ...
  private async sendEthereumTx(address: string): Promise<string> {
    const provider = await this.provider.ensureNetwork(this.state.activeNetwork);
  ...
  private async sendSolanaTx(address: string): Promise<string> {
    ...
    if (balance < 5000) {
      throw new Error(
        `Insufficient SOL balance. Need ~0.000005 SOL for fees.`
      );
    }
...
export function useMultiChainWalletState(): WalletContextType {
  ...
  // 创建辅助类实例
  const provider = new WalletProvider(ethWallets || [], solWallets || [], state.activeChain);
  ...
  const removeSolanaWallet = useCallback(async () => {
    Alert.alert('Info', 'Wallet removal not supported. It remains linked to your account.');
  }, []);
  ...
  const canSwitchTo = useCallback((chain: ChainType) => 
    chain === 'ethereum' ? !!ethAccount?.address : !!solAccount,
  [ethAccount, solAccount]);

  const getProvider = useCallback(() => provider.get(), [provider]);
  const getEthereumProvider = useCallback(() => provider.ensureNetwork(state.activeNetwork), [provider, state.activeNetwork]);
  ...
  const signMessage = useCallback(async (message: string) => {
    const address = state.activeChain === 'ethereum' 
      ? ethAccount?.address 
      : solAccount?.address;
